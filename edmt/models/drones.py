...
def airPoint(*args, **kwargs):
    raise RuntimeError(
        "airPoint() was removed in EDMT v1.0.5."
        "Use get_flight_routes() instead to generate flight routes."
        "Migration:\n"
            "airPoint(df) -> get_flight_routes(df)"
    )


def airLine(*args, **kwargs):
    raise RuntimeError(
        "airLine() was removed in EDMT v1.0.5."
        "Use get_flight_routes() instead to generate flight routes."
        "Migration:\n"
            "airLine(df) -> get_flight_routes(df)"
    )


def airSegment(*args, **kwargs):
    raise RuntimeError(
        "airSegment() was removed in EDMT v1.0.5."
    )
